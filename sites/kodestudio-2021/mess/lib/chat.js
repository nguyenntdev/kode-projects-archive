...
rtl.module("program",["System","SysUtils","JS","browserapp","kodefirebase","kodestorage","kodedom"],function () {
  "use strict";
  var $mod = this;
  ...
  this.loading = function (code) {
    ...
    if ($tmp === "init") {
      ...
      pas.System.Writeln("Code not found");
    };
  };
  this.stopchat = function (code) {
    var $tmp = code;
    if ($tmp === "init") {
      pas.System.Writeln("stopchat()")}
     else if ($tmp === "run") {}
    else {
      pas.System.Writeln("Code not found");
    };
  };
  this.send = function (code, text) {
    var $tmp = code;
    if ($tmp === "init") {
      pas.System.Writeln("send()")}
     else if ($tmp === "run") {
      $mod.mess.code = $mod.storage.getValue("tempCode");
      $mod.mess.from = $mod.storage.getValue("tempName");
      $mod.mess.message = text;
      $mod.fire.pushData("rooms\/" + $mod.mess.code,$mod.mess);
      $mod.obj.setValuehtml("input_message","");
    } else {
      pas.System.Writeln("Code not found");
    };
  };
